...
plotClusterQuality <- function(rca.obj, cluster.labels, width = 20, height = 20, folderpath = ".", filename = "RCA_Cluster_Quality.pdf") {
    ...
    nUMI_pMito_plot <- ggplot2::ggplot(data = quality.df, ggplot2::aes(x = nUMI, y = pMito)) + ggplot2::geom_point(size = 1) +ggplot2::geom_jitter() + ggplot2::facet_wrap(.~Cluster, scales = "free",nrow=5) + ggplot2::theme_bw() + ggplot2::ggtitle("nUMI vs pMito")+ ggplot2::geom_densit2d()
