...
#' @param nGene.thresholds numeric vector with lower and upper nGene thresholds (default: c(100, NULL)).
#' @param nUMI.thresholds numeric vector with lower and upper nUMI thresholds (default: c(100, NULL)).
#' @param percent.mito.thresholds numeric vector with lower and upper pMito thresholds (default: c(0.0, 0.2)).
#' @param min.cell.exp minimum number of cells a gene must be expressed in (default: 10)
#' @param plot boolean - if True, plot data filter metrics (default: TRUE).
#' @param filename file name of saved plots (default: "RCA_Filter.pdf")
...
#' print(RCA.pbmcs)
#' RCA.pbmcs <- dataFilter(RCA.pbmcs,nGene.thresholds = c(20,80),
#'  nUMI.thresholds = c(0,500), plot=FALSE)
#' print(RCA.pbmcs)

#' @export
#'
#'
dataFilter <- function(rca.obj,
                       nGene.thresholds = c(100, NULL),
                       nUMI.thresholds = c(1000, NULL),
                       percent.mito.thresholds = c(0.0, 0.2),
                       min.cell.exp = 10,
                       plot = T,
                       filename = "RCA_Filter.pdf") {
    if (!(("gridExtra" %in% base::.packages()) |
          !("ggplot2" %in% base::.packages()))) {
    ...
    if (!base::is.null(min.cell.exp)) {
        ...
        filt.genes <-
            base::rownames(data)[base::which(geneExpVec > min.cell.exp)]
    ...
    if (!base::is.null(nGene.thresholds)) {
        ...
        if (base::is.numeric(nGene.thresholds) &
            (base::length(nGene.thresholds) == 2)) {
            nGene.filt.cells <-
                filt.cells[base::which((nGeneVec >= nGene.thresholds[1]) &
                                           (nGeneVec <= nGene.thresholds[2]))]
        } else if (base::is.numeric(nGene.thresholds) &
                   (base::length(nGene.thresholds) == 1)) {
            nGene.filt.cells <-
                filt.cells[base::which(nGeneVec >= nGene.thresholds)]
        } else {
            warning(
                "nGene.thresholds was not of the appropriate format. Please enter a numeric vector with lower and upper thresholds."
            )
    ...
    if (!is.null(nUMI.thresholds)) {
        ...
        if (base::is.numeric(nUMI.thresholds) &
            (base::length(nUMI.thresholds) == 2)) {
            nUMI.filt.cells <-
                filt.cells[base::which((nUMIVec >= nUMI.thresholds[1]) &
                                           (nUMIVec <= nUMI.thresholds[2]))]
        } else if (base::is.numeric(nGene.thresholds) &
                   (base::length(nUMI.thresholds) == 1)) {
            nUMI.filt.cells <-
                filt.cells[base::which(nUMIVec >= nUMI.thresholds)]
        } else {
            warning(
                "nUMI.thresholds was not of the appropriate format. Please enter a numeric vector with lower and upper thresholds."
            )
    ...
    filt.cells <-
        base::intersect(base::intersect(nGene.filt.cells, nUMI.filt.cells),
                        pMito.filt.cells)
    ...
    if (plot) {
        ...
        cellFiltDf$isFilt <-
            base::ifelse(
                test = base::rownames(cellFiltDf) %in% filt.cells,
                yes = "Retained",
                no = "Discarded"
            )
        ...
        nGene.thres.lines <-
            base::lapply(nGene.thresholds, function(thres) {
                ggplot2::geom_vline(
                    ggplot2::aes(xintercept = thres),
                    color = "red",
                    linetype = "dashed",
                    size = 1
                )
        ...
        pMito.thres.lines <-
            base::lapply(percent.mito.thresholds, function(thres) {
                ggplot2::geom_hline(
                    ggplot2::aes(yintercept = thres),
                    color = "red",
                    linetype = "dashed",
                    size = 1
                )
        ...
        nGene_pMito_plot <-
            ggplot2::ggplot(data = cellFiltDf, ggplot2::aes(
                x = nGene,
                y = pMito,
                colour = isFilt
            )) +
        ...
        # Create threshold lines
        nUMI.thres.lines <-
            base::lapply(nUMI.thresholds, function(thres) {
                ggplot2::geom_vline(
                    ggplot2::aes(xintercept = thres),
                    color = "red",
                    linetype = "dashed",
        ...
        nUMI_pMito_plot <-
            ...
            ggplot2::ggtitle("b) nUMI vs pMito") + ggplot2::theme(legend.position =
                                                                      "none")
        ...
        nGene_nUMI_plot <-
            ...
            ggplot2::theme_bw() + ggplot2::ggtitle("c) nGene vs nUMI") + ggplot2::theme(legend.position =
                                                                                            "right") +
    ...
    rca.obj$raw.data <-
        methods::as(data[filt.genes, filt.cells], "dgCMatrix")
