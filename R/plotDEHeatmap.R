...
#' @param scale TRUE if genes should be z-transformed before plotting (default FALSE).
#' @param width width of plot in inches (default is 20).
#' @param height height of plot in inches (default is 20).
#' @param filename file name of saved heatmap (default RCA_DE_Heatmap)
#' @param extraCellProperty vector indicating cell property to be plotted in heatmap (default NULL)
#' @param SeuratColorScheme Use the color scheme known from Seurat (default TRUE). Otherwise use a blue to red color scheme.
#'
#' @examples
#' \dontrun{
...
#' plotDEHeatmap(RCA.pbmcs, scale = TRUE)
#' }
#' @export
#'
...
plotDEHeatmap <-
    function(rca.obj,
             ...
             SeuratColorScheme = TRUE) {
        # Extract projection data and clustering result from RCA object
        de.genes <-
            base::unique(base::as.character(rca.obj$DE.genes$Top.DE.genes$Gene))
        cellTree = rca.obj$clustering.out$cellTree
        clusterColorList = rca.obj$clustering.out$dynamicColorsList

        heatmapIn <-
            base::matrix(0, base::length(de.genes), base::dim(rca.obj$data)[2])
        for (i in base::c(1:base::length(de.genes))) {
            heatmapIn[i, ] <-
                rca.obj$data[base::which(base::row.names(rca.obj$data) == de.genes[i]), ]
        }
        ...
        if (scale) {
            if (SeuratColorScheme) {
                colorScheme <-
                    circlize::colorRamp2(
                        base::c(base::min(heatmapIn), 0, base::max(heatmapIn)),
                        base::c("magenta", "black", "yellow")
                    )
            } else{
                colorScheme <-
                    circlize::colorRamp2(base::c(base::min(heatmapIn), 0, base::max(heatmapIn)),
                                         base::c("#7777FF",
                                                 "white",
                                                 "red"))
            }
            ...
            if (SeuratColorScheme) {
                colorScheme <-
                    grDevices::colorRampPalette(base::c("black", "yellow"))(256)
            } else{
                colorScheme <-
                    circlize::colorRamp2(base::c(base::min(heatmapIn), base::max(heatmapIn)),
                                         base::c("white",
                                                 "red"))
            }
        }
        if ((base::class(cellTree) == "hclust") &&
            (base::length(cellTree$order) == base::ncol(heatmapIn))) {
            ...
            if (base::is.null(clusterColorList)) {
                ...
                base::names(clusterColorDf) <-
                    base::names(clusterColorList)
                ...
                mito.genes = base::grep(
                    pattern = "^MT-|^Mt-",
                    x = base::rownames(rca.obj$raw.data),
                    value = T
                )
                    ...
                    if (base::is.null(extraCellProperty)) {
                        ...
                        cellPropertyList <-
                            base::list(
                                "nUMI" = nUMI,
                                "NODG" = nodg,
                                "extra" = extraCellProperty
                            )
                    }
                } else {
                    pMito <-
                        Matrix::colSums(rca.obj$raw.data[mito.genes,]) / Matrix::colSums(rca.obj$raw.data)
                    if (base::is.null(extraCellProperty)) {
                        cellPropertyList <-
                            base::list(
                                "nUMI" = nUMI,
                ...
                annoBarPlotList <-
                    base::lapply(cellPropertyList, function(cellPropertyVec) {
                ...
                base::names(annoBarPlotList) <-
                    base::names(cellPropertyList)
                ...
                columnColorBar <-
                    base::do.call(what = ComplexHeatmap::HeatmapAnnotation,
                                  args = paramList)
