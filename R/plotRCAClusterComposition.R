...
plotRCAClusterComposition <- function(rca.obj, deepSplit=1, folderpath = ".", filename = "Cluster_Composition.pdf") {
	...
	index=base::which(clusterColors==type)
        enrichmentAll<-base::rbind(enrichmentAll,
                                   (base::cbind(type,base::table(base::unlist(rca.obj$cell.Type.Estimate)[index]))))
    }
    ...
    enrichmentAll<-base::cbind(enrichmentAll,enrichmentAll$Count/enrichmentAll$n*100)
    ...
    if (("randomcoloR" %in% base::.packages())){
       dColors<-randomcoloR::distinctColorPalette(base::length(base::unique(enrichmentAll$CT)))
       nCols<-ceiling(length(unique(enrichmentAll$CT))/26)
       ratioPlot<-ggplot2::ggplot(enrichmentAll,ggplot2::aes(x=Cluster,y=Ratio,fill=CT))+
           ggplot2::geom_bar(stat="identity")+
           ggplot2::theme_bw(15)+ggplot2::ylab("Percentage")+
           ggplot2::coord_flip()+ggplot2::ggtitle("a)")+
           ggplot2::theme(legend.position = "none")+ggplot2::scale_fill_manual(values=dColors)
       countPlot<-ggplot2::ggplot(enrichmentAll,ggplot2::aes(x=Cluster,y=Count,fill=CT))+
           ggplot2::geom_bar(stat="identity")+ggplot2::theme_bw(15)+
           ggplot2::ylab("Count")+
           ggplot2::coord_flip()+ggplot2::ggtitle("b)")+ggplot2::scale_fill_manual(values=dColors)+
           ggplot2::labs(fill="Cell type")+ggplot2::guides(fill=ggplot2::guide_legend(ncol=nCols))
    }
    else{
       ratioPlot<-ggplot2::ggplot(enrichmentAll,ggplot2::aes(x=Cluster,y=Ratio,fill=CT))+
           ggplot2::geom_bar(stat="identity")+ggplot2::theme_bw(15)+ggplot2::ylab("Percentage")+
           ggplot2::coord_flip()+ggplot2::ggtitle("a)")+ggplot2::theme(legend.position = "none")
       countPlot<-ggplot2::ggplot(enrichmentAll,ggplot2::aes(x=Cluster,y=Count,fill=CT))+
           ggplot2::geom_bar(stat="identity")+ggplot2::theme_bw(15)+ggplot2::ylab("Count")+
           ggplot2::coord_flip()+ggplot2::ggtitle("b)")+ggplot2::labs(fill="Cell type")+
           ggplot2::guides(fill=ggplot2::guide_legend(ncol=nCols))
    }
   grDevices::pdf(base::paste0(folderpath,"/Heatmap_Composition_",filename),width=15+(nCols-1),height=7)
   gridExtra::grid.arrange(ratioPlot,countPlot,widths=c(1,(1.4+(nCols-1)/10),nrow=1))
   grDevices::dev.off()
}else{
