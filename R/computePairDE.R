...
dataDE <- function(rca.obj, 
	...
	if (class(rca.obj$clustering.out$cellTree)=="hclust"){
		...
		names(remap)<-unique(rca.obj$clustering.out$dynamicColorsList[[deepsplit]])
	...
	for (clusteri in 0:(total.clus-2)){
		for (clusterj in (clusteri+1):(total.clus-1)){
			...
			marker.genes = ComputePairWiseDE(object = rca.obj$data,
			cells.1 = cells.1,
			cells.2 = cells.2,
			features = NULL,
	...
	######################################
	mC1<-df %>% group_by(group1,group2) %>% dplyr::top_n(n = topGenesPerCluster, wt = avg_logFC)
	markers2<-df
	markers2$avg_logFC<-(-1)*(markers2$avg_logFC)
	mC2<-markers2 %>% group_by(group2,group1) %>% dplyr::top_n(n = topGenesPerCluster, wt = avg_logFC)
	topMarkers<-data.frame(rbind(cbind(Cluster=mC1$group1,Gene=mC1$gene),cbind(Cluster=mC2$group2,Gene=mC2$gene)))
